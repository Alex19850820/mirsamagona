<?php
class ControllerExtensionModuleSeoManager extends Controller {
    private $error = array();

    public function index() {
        $this->load->language('extension/module/seo_manager');

        $this->document->setTitle($this->language->get('heading_title'));

        $this->load->model('extension/module/seo_manager');
    ...
    protected function getForm() {
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href' => $this->url->link('extension/module/seo_manager', 'token=' . $this->session->data['token'] . $url, true),
        );

        $data['delete'] = $this->url->link('extension/module/seo_manager/delete', 'token=' . $this->session->data['token'] . $url, true);

        $data['save'] = $this->url->link('extension/module/seo_manager/update', 'token=' . $this->session->data['token'] . $url, true);

        $data['clear'] = $this->url->link('extension/module/seo_manager/clear', 'token=' . $this->session->data['token'] . $url, true);
        ...
        $url_alias_total = $this->model_extension_module_seo_manager->getTotalUrlAalias($filter_data);

        $results = $this->model_extension_module_seo_manager->getUrlAaliases($filter_data);
        ...
        $data['sort_query'] = $this->url->link('extension/module/seo_manager', 'token=' . $this->session->data['token'] . '&sort=query' . $url, true);
        $data['sort_keyword'] = $this->url->link('extension/module/seo_manager', 'token=' . $this->session->data['token'] . '&sort=keyword' . $url, true);
        ...
        $pagination->url = $this->url->link('extension/module/seo_manager', 'token=' . $this->session->data['token'] . $url . '&page={page}', true);
        ...
        $this->response->setOutput($this->load->view('extension/module/seo_manager', $data));
    ...
    public function update() {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
            $this->model_extension_module_seo_manager->updateUrlAlias($this->request->post);
            $this->session->data['success'] = $this->language->get('text_success');

            $this->response->redirect($this->url->link('extension/module/seo_manager', 'token=' . $this->session->data['token'] . $url, true));
        }
    ...
    public function delete() {
        ...
        if (isset($this->request->post['selected']) && $this->validate()) {
            foreach ($this->request->post['selected'] as $url_alias_id) {
                $this->model_extension_module_seo_manager->deleteUrlAlias($url_alias_id);
            }
    ...
    protected function validatePermission() {
        return $this->user->hasPermission('modify', 'extension/module/seo_manager');
    }
}